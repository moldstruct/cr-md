...
// printf("c6[i]= %lf,c12[i]= %lf, ATOM_Z[i]= %i \n",sigma_array[1],epsilon_array[1],ATOM_Z[1]);
	...
	//				morse_force = 0;
		...
		///			}
					...
					c6               = (lj_c6_factor*vdwparam[tj]);   // dont change parametsr for c6/c12
		            c12              = (lj_c12_factor*vdwparam[tj+1]);
			...
			//		}
				...
				if (Vvdwtot > 1e+05) { 
				 ...
				 // printf("LJ_factor14 = %lf \n", lj4_factor);
					...
					c6               = LJ_factor*(lj_c6_factor*vdwparam[tj]);   // dont change parametsr for c6/c12
		            c12              = LJ_factor*(lj_c12_factor*vdwparam[tj+1]); //
