...
real do_listed_vdw_q(int ftype, int nbonds,
                     ...
                     int *global_atom_index)
{
    ...
    float lj4_factor = 1.0;
    gmx_gbdata_t gbdata;
    gbdata.lj_param = lj4_factor;
    ...
    for (i = 0; (i < nbonds);)
    {
        ...
        //   if  (md->chargeA[aj]>8 ||  md->chargeA[ai] > 8) {
        // printf("md->chargeA[ai]= %lf, md->chargeA[aj]= %lf\n", md->chargeA[ai], md->chargeA[aj]);
        // printf("md->massT[ai]= %lf, md->massT[aj]= %lf md->typeA[ai]= %i, md->massT[aj]= %i, ATOM_Z[md->typeA[ai]]= %i ATOM_Z[md->typeA[aj]]= %i\n", round(md->massT[ai]), round(md->massT[aj]), md->typeA[ai], md->typeA[aj], ATOM_Z[md->typeA[ai]], ATOM_Z[md->typeA[aj]]);

        if ((ATOM_Z[md->typeA[aj]] - md->chargeA[aj]) < 0 || (ATOM_Z[md->typeA[ai]] - md->chargeA[ai]) < 0)
        {
            lj4_factor = 1.0;
            printf("Negative\n");
        ...
        else
        {
            ...
            if (ATOM_Z[md->typeA[ai]] == 1222 && ATOM_Z[md->typeA[aj]] == 1222)
            {
            ...
            else
            {
                ...
                lj4_factor = 0.5 * (((ATOM_Z[md->typeA[ai]] - md->chargeA[ai]) / ATOM_Z[md->typeA[ai]]) + ((ATOM_Z[md->typeA[aj]] - md->chargeA[aj]) / ATOM_Z[md->typeA[aj]]));
                // printf("FAC = %lf\n", lj4_factor);
            }
        }
